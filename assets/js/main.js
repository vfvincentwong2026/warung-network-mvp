 ...
 *   1. 非地图 UI（导航/数字动画/档期/滚动渐显）立即初始化；
 *   2. 地图【先用演示数据立即渲染】（角标显示 Demo 标注），页面秒开；
 *   3. SheetLoader.load() 异步返回真实数据后，热替换 marker/漏斗/大学 popup
 *      并翻转角标为「🟢 实时数据」；返回 null 则保持演示数据不变。
 *
 * 性能：地图开 preferCanvas，Warung 点位用 L.circleMarker（canvas 渲染），
 *      活跃点位的脉冲环用独立 divIcon overlay（数量极少），222+ 点位不卡顿。
 * ============================================================ */
(function () {
  'use strict';
  ...
  function initMap(initialWarungs) {
    ...
    // preferCanvas：大量点位时渲染到 canvas，避免 DOM divIcon 卡顿
    ...
    // CartoDB Dark Matter 深色底图（零 API key）
    ...
    var warungMarkers = []; // { id, universityId, status, marker, pulse }（随数据替换重建）
    ...
    /* --- 大学徽章 marker（divIcon，仅 9 个，DOM 开销可忽略） --- */
    ...
    function renderWarungs() {
      ...
      warungs.forEach(function (w) {
        ...
        warungMarkers.push({ id: w.id, universityId: w.universityId, status: w.status, marker: m, pulse: pulse });
      });
    }
    ...
    function buildWarungPopup(w, st) {
      ...
      // target 状态底色为半透明白，文字需浅色（对应 .popup-status--light）
      ...
      return '<div class="popup-title">' + escapeHtml(w.name) + '</div>' +
        ...
        '<span class="' + statusCls + '" style="background:' + escapeHtml(st.color) + '">' + escapeHtml(st.label) + '</span>';
    }
    renderWarungs();
      ...
      Object.keys(STATUS).forEach(function (key, i, arr) {
        ...
        funnelHtml +=
          '<div class="funnel__stage">' +
            ...
            '<span class="funnel__label">' + escapeHtml(STATUS[key].label) + '</span>' +
          '</div>';
    ...
    var legendHtml = '<div class="map-legend__title">图例</div>' +
      '<div class="map-legend__item"><span class="map-legend__swatch map-legend__swatch--uni"></span>大学（点击飞到校区）</div>';
    ...
    allChip.style.setProperty('--chip-color', '#EDEDED');
      ...
      warungMarkers.forEach(function (wm) {
        ...
        if (hit) {
          wm.marker.setStyle(highlighted
            ? { opacity: 1, fillOpacity: 1, color: '#E8611C', weight: 2 }  // 选中大学：强调色描边
            : dotStyle(wm.status));
        } else {
          wm.marker.setStyle({ opacity: 0.12, fillOpacity: 0.1, color: 'rgba(255,255,255,0.25)', weight: 1 });
        }
        // 脉冲环同步淡化
        if (wm.pulse) {
